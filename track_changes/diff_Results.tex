...
\textbf{DexYCB Dataset} \cite{chao2021dexycb}. The DexYCB dataset is highly suitable for evaluating hand-held object pose tracking in videos due to its comprehensive set of annotated hand-object interactions. The dataset includes over 582 video sequences \DIFdelbegin \DIFdel{that capture }\DIFdelend \DIFaddbegin \DIFadd{capturing }\DIFaddend the dynamic manipulation of 20 different objects from the YCB dataset by 10 subjects. Each sequence provides 6D object poses, 3D hand poses, and RGB-D data, which are crucial for testing the robustness and accuracy of tracking algorithms in real-world conditions. The variety of objects, coupled with \DIFdelbegin \DIFdel{the }\DIFdelend naturalistic hand movements, makes DexYCB an ideal benchmark for assessing how well a model can maintain accurate pose estimates over time, particularly in scenarios involving rapid hand movements and frequent occlusions. \DIFaddbegin \DIFadd{To prepare the dataset, we follow the benchmark setup $S0$ (default) as defined in \cite{chao2021dexycb}. The train split contains sequences from all 10 subjects, all 8 camera views, and all 20 grasped objects. We ensure that no sequences are shared between the training, validation, and test splits. Images are resized to $256 \times 256$ pixels, and data augmentation techniques, including random rotation, scaling, and brightness adjustments, are applied during training to enhance model generalization. 
}\DIFaddend 

\noindent \textbf{FPHAB Dataset} \cite{garcia2018first}. The First-Person Hand Action Benchmark (FPHAB) dataset is particularly relevant for evaluating pose tracking in first-person view scenarios. It offers 1175 RGB-D video sequences across 45 different action classes, all annotated with 3D hand poses. This dataset is valuable for testing hand-held object pose tracking models because the egocentric viewpoint introduces challenges such as motion blur, self-occlusion, and varying lighting conditions, which are common in real-world applications. By providing a diverse set of actions and interactions, FPHAB enables the assessment of a model's ability to track objects consistently despite the inherent complexities of first-person perspectives. \DIFaddbegin \DIFadd{In our setup, the dataset is preprocessed by resizing images to $256 \times 256$ pixels. We split the dataset into approximately 70,000 frames for training and 30,000 frames for testing. Data augmentation, including random flipping, cropping, and color jitter, is applied to the training set. To evaluate the model's robustness to occlusions, more than 70\% of the test set images involve occlusions, with at least 50\% of those under heavy occlusion.
}\DIFaddend 

\noindent \textbf{HO-3D Dataset} \cite{hampali2020honnotate}. This dataset is specifically designed to evaluate 3D hand-object pose estimation, making it an excellent benchmark for hand-held object pose tracking in videos. With over 80,000 annotated frames, this dataset captures intricate hand-object interactions, often under challenging conditions like severe occlusions and complex background scenes. The sequences in HO-3D are recorded in real-world environments, providing a realistic testbed for assessing the effectiveness of pose tracking algorithms. The dataset's emphasis on naturalistic hand movements and object manipulation in cluttered environments is critical for evaluating how well a model can maintain accurate pose estimates across frames when faced with partial or full occlusions. \DIFaddbegin \DIFadd{We preprocess the dataset by resizing all images to $256 \times 256$ pixels and applying data augmentation such as random rotation, scaling, and brightness adjustments to enhance diversity in the training data. The dataset is split into approximately 65,000 frames for training and 10,000 frames for testing, with over 70\% of the test set images involving occlusions and at least 50\% of those under heavy occlusion.
}\DIFaddend 

\subsection{Implementation Details}

Our implementation\footnote{Our code and other materials are available at \url{https://github.com/hoangcuongbk80/6dInHandVid}} employs ResNet-50 \cite{he2016deep} combined with ROIAlign \cite{he2017mask} to extract object features at an output resolution of $32 \times 32$ with 256 feature channels. The spatial transformer module, which includes both the encoder and decoder, is built with 3 layers of multi-head self-attention\DIFdelbegin \DIFdel{, each layer containing }\DIFdelend \DIFaddbegin \DIFadd{. Each attention layer contains }\DIFaddend 8 \DIFdelbegin \DIFdel{attention headsand }\DIFdelend \DIFaddbegin \DIFadd{heads, with }\DIFaddend a feed-forward network dimension of 256. \DIFdelbegin \DIFdel{In the transformer decoder , the query vectors are also set to a dimension of 256. The }\DIFdelend \DIFaddbegin \DIFadd{The transformer decoder uses query vectors with a dimensionality of 256, and the }\DIFaddend temporal transformer shares the same architecture as the spatial transformer. \DIFdelbegin \DIFdel{While the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend ResNet-50 backbone is initialized with \DIFdelbegin \DIFdel{weights pre-trained on ImageNet, }\DIFdelend \DIFaddbegin \DIFadd{ImageNet-pretrained weights, while }\DIFaddend the transformer components are trained from scratch using an end-to-end approach. \DIFdelbegin \DIFdel{We utilize }\DIFdelend \DIFaddbegin \DIFadd{Training is performed over 120 epochs using }\DIFaddend the Adam optimizer with an initial learning rate of $1 \times 10^{-5}$. \DIFdelbegin \DIFdel{Following the adversarial training protocol, }\DIFdelend \DIFaddbegin \DIFadd{The learning rate is reduced by 30\% every 20 epochs. To enhance robustness, adversarial training is employed, where }\DIFaddend the transformers are alternately updated at each training step. \DIFdelbegin \DIFdel{Training is conducted over 120 epochs, with the learning rate being reduced by 30\% every 20 epochs. The model is trained }\DIFdelend \DIFaddbegin \DIFadd{Batch size is set to 64, and training is conducted }\DIFaddend across 8 NVIDIA RTX 2080Ti GPUs. \DIFaddbegin \DIFadd{During training, the dataset is augmented with random rotation, scaling, color jitter, and horizontal flipping to improve generalization. The model is evaluated using standard metrics, including ADD(-S) and AUC for 6D pose estimation, to measure accuracy across different datasets. }\DIFaddend For inference, the trained model is deployed on a single \DIFaddbegin \DIFadd{NVIDIA }\DIFaddend RTX 2080Ti GPU.
...
\begin{figure*}[!ht]
    ...
    \hfill
    \begin{subfigure}{0.98\textwidth}
        \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\linewidth]{figs/1_0}
%DIFDELCMD <         %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Ground truth}}
    %DIFAUXCMD
%DIFDELCMD < \end{subfigure}
%DIFDELCMD <     \hfill
%DIFDELCMD <     \begin{subfigure}{0.98\textwidth}
%DIFDELCMD <         %%%
...
%DIFDELCMD <     \begin{subfigure}{0.98\textwidth}
%DIFDELCMD <         \includegraphics[width=\linewidth]{figs/1_3}
%DIFDELCMD <         %%%
\DIFdelendFL \caption{\DIFdelbeginFL \DIFdelFL{Castro et al}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Qualitative Comparison of the state-of-the-art video-based method \cite{wang2023deep} and our method. The proposed method robustly generates more stable and accurate poses over time than previous approaches. (a) are the input RGB images. (b-c) shows the rendered images using predicted object poses and colorcoded visualization of point-wise distance error}\DIFaddendFL .\DIFdelbeginFL \DIFdelFL{\cite{castro2023crt}}\DIFdelendFL }
    \DIFdelbeginFL %DIFDELCMD < \end{subfigure}
%DIFDELCMD <     %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Qualitative Comparison of the state-of-the-art single-view method \cite{castro2023crt}, video-based method \cite{wang2023deep},
...
\begin{figure*}[!ht]
    ...
    \hfill
    \begin{subfigure}{0.98\textwidth}
        \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\linewidth]{figs/2_0}
%DIFDELCMD <         %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Ground truth}}
...
\DIFdelendFL \includegraphics[width=\linewidth]{figs/2_1}
    ...
    \DIFdelbeginFL %DIFDELCMD < \hfill
%DIFDELCMD <     \begin{subfigure}{0.98\textwidth}
%DIFDELCMD <         \includegraphics[width=\linewidth]{figs/2_3}
%DIFDELCMD <         %%%
\DIFdelendFL \caption{\DIFdelbeginFL \DIFdelFL{Castro et al}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Qualitative Comparison of the state-of-the-art video-based method \cite{wang2023deep}, and our method. Here we only show results on frames under heavy occlusion (frames with a visibility score \( v_i \) smaller than \( \delta = 0.5 \)). (a) are the input RGB images. (b-c) shows the rendered images using predicted object poses and colorcoded visualization of point-wise distance error}\DIFaddendFL .\DIFdelbeginFL \DIFdelFL{\cite{castro2023crt}}\DIFdelendFL }
    \DIFdelbeginFL %DIFDELCMD < \end{subfigure}
%DIFDELCMD <     %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
...
\DIFaddbegin \subsection{\DIFadd{Comparison with Transformer-Based Methods}}

%DIF > ----------------------------------------------------------
\begin{table*}[h]
\caption{\DIFaddFL{Performance comparison of our method with existing transformer-based approaches for 6D object pose estimation across three benchmark datasets. We only evaluate methods on frames under heavy occlusion (frames with a visibility score \( v_i \) smaller than \( \delta = 0.5 \)).}}
\label{tab:Transformer-Based}
\begin{center}
\begin{tabular}{l c c c c c c c} 
\hline
& \multicolumn{2}{c}{DexYCB} & \multicolumn{2}{c}{FPHAB} & \multicolumn{2}{c}{HO-3D} & \DIFaddFL{Time }\\
\hline
\DIFaddFL{Method }& \DIFaddFL{$AUC$ }& \DIFaddFL{$AP$ }& \DIFaddFL{$AUC$ }& \DIFaddFL{$AP$ }& \DIFaddFL{$AUC$ }& \DIFaddFL{$AP$ }& \DIFaddFL{$ms$ }\\  
\hline 

\DIFaddFL{Zhou et al. \cite{zhou2023deep} }& \DIFaddFL{61.6 }& \DIFaddFL{59.7 }& \DIFaddFL{62.8 }& \DIFaddFL{63.0 }& \DIFaddFL{62.1 }& \DIFaddFL{63.9 }& \DIFaddFL{82 }\\

\DIFaddFL{Zhou et al. \cite{zhou2023deep} + CosyPose \cite{labbe2020cosypose} }& \DIFaddFL{69.1 }& \DIFaddFL{67.2 }& \DIFaddFL{70.1 }& \DIFaddFL{71.3 }& \DIFaddFL{70.4 }& \DIFaddFL{71.3 }& \DIFaddFL{98 }\\

\DIFaddFL{Hong et al. \cite{hong2024transformer} }& \DIFaddFL{61.9 }& \DIFaddFL{60.2 }& \DIFaddFL{63.7 }& \DIFaddFL{64.3 }& \DIFaddFL{63.5 }& \DIFaddFL{64.2 }& \DIFaddFL{86 }\\

\DIFaddFL{Hong et al. \cite{hong2024transformer} + CosyPose \cite{labbe2020cosypose} }& \DIFaddFL{71.2 }& \DIFaddFL{69.6 }& \DIFaddFL{71.0 }& \DIFaddFL{73.7 }& \DIFaddFL{71.1 }& \DIFaddFL{72.0 }& \DIFaddFL{93 }\\

\DIFaddFL{Li et al. \cite{li2023depth} }& \DIFaddFL{58.0 }& \DIFaddFL{56.7 }& \DIFaddFL{60.0 }& \DIFaddFL{61.7 }& \DIFaddFL{60.8 }& \DIFaddFL{61.5 }& \DIFaddFL{67 }\\

\DIFaddFL{Li et al. \cite{li2023depth} + CosyPose \cite{labbe2020cosypose} }& \DIFaddFL{65.5 }& \DIFaddFL{64.2 }& \DIFaddFL{68.3 }& \DIFaddFL{69.0 }& \DIFaddFL{66.2 }& \DIFaddFL{68.4 }& \DIFaddFL{84 }\\

\DIFaddFL{Periyasamy et al. \cite{periyasamy2023yolopose} }& \DIFaddFL{59.1 }& \DIFaddFL{58.2 }& \DIFaddFL{61.0 }& \DIFaddFL{61.9 }& \DIFaddFL{60.8 }& \DIFaddFL{61.5 }& \DIFaddFL{69 }\\

\DIFaddFL{Periyasamy et al. \cite{periyasamy2023yolopose} + CosyPose \cite{labbe2020cosypose} }& \DIFaddFL{66.3 }& \DIFaddFL{67.1 }& \DIFaddFL{68.8 }& \DIFaddFL{69.0 }& \DIFaddFL{66.2 }& \DIFaddFL{67.2 }& \DIFaddFL{79 }\\

\DIFaddFL{Ours }& \DIFaddFL{\textbf{78.3} }& \DIFaddFL{\textbf{78.5} }& \DIFaddFL{\textbf{79.4} }& \DIFaddFL{\textbf{80.1} }& \DIFaddFL{\textbf{79.6} }& \DIFaddFL{\textbf{80.8} }& \DIFaddFL{66 }\\
\hline
\end{tabular}
\end{center}
\end{table*}
